...
Gathering rewards arrive in one-stone commodity deeds, including each reagent type. Ore rewards are iron ingots because native raw ore cannot be deeded. Redeem deeds in a bank or secured commodity deed box. Pet tickets, bonus supplies, and equipment remain usable items.

Thunderstorm now works against a single nearby hostile when mana permits, with a six-second selection cooldown. It is cast on the companion itself so losing a selected enemy mid-cast does not cancel the area spell. Native Wildfire remains unavailable.

Validation: 149 real-map local tests, including idle transitions, AFK/combat/disconnect, manual mission isolation, skill eligibility, deed contents, and existing reward replay safeguards.
