...
//   * [GuildStorageKit gives staff a one-click setup kit.
...
namespace Server.CustomBots
{
    ...
    {
        internal static readonly HashSet<GuildProfessionChest> Registry = new();
        [AfterDeserialization] private void Register() { Registry.Add(this); }
        public override void OnDelete() { Registry.Remove(this); MasterChest = null; base.OnDelete(); }
    ...
    public partial class GuildMasterChest : MetalChest
    {
        ...
        internal bool CanAccess(Mobile from, bool requireNearby = true)
        {
            if (Deleted || from?.Deleted != false || !from.Alive) { return false; }
            if (IsChildOf(from.Backpack)) { return true; }
            var house = BaseHouse.FindHouseAt(this);
            return house != null && (house.IsOwner(from) || house.IsCoOwner(from) || house.IsGuildMember(from)) &&
                (!requireNearby || from.Map == Map && from.InRange(this, 3) && from.InLOS(this));
        }


        public bool TryDeposit(Mobile from, Item item, bool sendMessage = false)
        {
            if (!CanAccess(from) || item == null || item.Deleted)
            {
                return false;
            }
        ...
        public int RelinkNearby()
        {
            ...
            foreach (var item in Map.GetItemsInRange(GetWorldLocation(), EffectiveSortRange))
            {
                ...
                if ((chest.MasterChest == null || chest.MasterChest.Deleted) && BaseHouse.FindHouseAt(chest) == BaseHouse.FindHouseAt(this))
                {
                    chest.Relink(this);
                    linked++;
                }
        ...
        public List<GuildProfessionChest> FindLinked(bool worldWide = false)
        {
            ...
            if (worldWide)
            {
                foreach (var item in GuildProfessionChest.Registry)
                {
        ...
        private bool TryRole(Mobile from, Item item, GuildStorageRole role)
        {
            ...
            {
                if (chest.Role != role || BaseHouse.FindHouseAt(chest) != BaseHouse.FindHouseAt(this))
                {
                    continue;
                }
    ...
    public static class GuildStorageClassifier
    {
        ...
        public static GuildStorageRole Classify(Item item)
        {
            ...
            if (item is CommodityDeed deed) { return deed.Commodity == null ? GuildStorageRole.Resources : Classify(deed.Commodity); }
            if (item is Server.UOOffline.HavenPetLeash or Server.UOOffline.HavenBondingPotion or Server.UOOffline.HavenExpeditionPetClaim)
    ...
    public static class GuildStorageCommands
    {
        ...
        {
            CommandSystem.Register("GuildStorageKit", AccessLevel.GameMaster, Kit_OnCommand);
            ...
            CommandSystem.Register("GuildStorageMissing", AccessLevel.GameMaster, Missing_OnCommand);
        ...
        private sealed class GuildStorageTarget : Target
        {
            ...
            {
                if (targeted is not GuildMasterChest master || !master.CanAccess(from))
                {
