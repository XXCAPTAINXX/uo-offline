...
{
    internal static readonly HashSet<HavenSnowBearDen> Registry = new();
    internal static readonly Point3D Site = new(942, 116, 0);
    [SerializableField(0)] private HavenSnowBear _bear;
    ...
    internal static bool TryFloor(Point3D center, Map map, out Point3D point)
    {
        ...
        while (pending.TryDequeue(out var current))
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                if (Math.Abs(next.Z - current.Z) <= 2 && map.CanFit(next, 16, checkMobiles: false)) { pending.Enqueue(next); }
            }
        }
    ...
    internal void Tick()
    {
        ...
        { Bear = null; NextSpawn = Core.Now + TimeSpan.FromMinutes(5); }
        if (Bear == null)
        {
            if (Core.Now < NextSpawn || !TryFloor(Location, Map, out var point)) { return; }
            var bear = new HavenSnowBear(); var roll = Utility.RandomDouble(); var tier = roll < .50 ? 1 : roll < .85 ? 2 : 3;
            HavenPetRarity.Apply(bear, tier); bear.MinTameSkill = tier == 3 ? 120 : tier == 2 ? 110 : 100;
