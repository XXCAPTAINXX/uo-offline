...
public partial class HavenResourceSatchel : Bag
{
    ...
    internal static bool Accepts(Item item) => item is not Container && (HavenResourceCatalog.Index(item) >= 0 || item is BaseOre or Bandage or Fish or RawFishSteak);
    public override bool TryDropItem(Mobile from, Item item, bool message)
    {
        if (!Accepts(item)) { if (message) { from.SendMessage("This satchel holds crafting resources, fish and bandages, not equipment or other bags."); } return false; }
    ...
    public override void GetProperties(IPropertyList list)
    { base.GetProperties(list); list.Add($"{"Resource weight reduced by 90%; normal item count limits apply."}"); }
}
