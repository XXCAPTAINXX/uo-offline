...
namespace Server.Spells.SkillMasteries
{
	public abstract class SkillMasterySpell : Spell
	{
        ...
        public override void OnBeginCast() { base.OnBeginCast(); SendCastEffect(); }
		...
		public override bool CheckCast()
		{
            ...
            else if (Caster is PlayerMobile && MasteryProgress.Current(Caster) != CastSkill)
		...
		public virtual bool OnTick()
		{
            ...
            // This ML shard does not implement later-era negative item attributes.

            if (Caster.Deleted || (Caster is PlayerMobile && Caster.NetState == null) || Expires < Core.Now || !Caster.Alive || Caster.IsDeadBondedPet)
            {
                Expire();
            }
            else if (Target != null && (Target.Deleted || !Target.Alive || Target.Map != Caster.Map))
            {
                Expire();
            }
        ...
        {
            if (Caster.Deleted || !Caster.Alive) { return; }
            PartyList ??= new List<Mobile>();
            for (var i = PartyList.Count - 1; i >= 0; i--)
            {
                var member = PartyList[i];
                if (!EligibleRecipient(member, playersOnly))
                {
                    PartyList.RemoveAt(i);
                    if (!member.Deleted) { RemovePartyEffects(member); }
                }
            }
            if (!PartyList.Contains(Caster)) { AddPartyMember(Caster); }
            if (!PartyEffects || Caster.Map == null || Caster.Map == Map.Internal) { return; }
            foreach (var member in Caster.Map.GetMobilesInRange<Mobile>(Caster.Location, PartyRange))
            {
                if (!PartyList.Contains(member) && EligibleRecipient(member, playersOnly)) { AddPartyMember(member); }
            }
        }
        private bool EligibleRecipient(Mobile member, bool playersOnly)
        {
            if (member?.Deleted != false || !ValidPartyMember(member)) { return false; }
            if (member == Caster) { return true; }
            if (!PartyEffects || playersOnly && member is not PlayerMobile) { return false; }
            var owner = member is BaseCreature pet ? pet.GetMaster() ?? member : member;
            var casterOwner = Caster is BaseCreature casterPet ? casterPet.GetMaster() ?? Caster : Caster;
            var party = Party.Get(casterOwner);
            return owner == casterOwner || party != null && Party.Get(owner) == party;
        }
        ...
        protected void AddPartyMember(Mobile member)
        {
            PartyList.Add(member);
            AddPartyEffects(member);
        }
        protected void RemovePartyMember(Mobile member)
        {
        ...
        public static SkillMasterySpell GetHarmfulSpell(Mobile target, Type type)
        {
            foreach (Mobile m in m_Table.Keys)
            {
                ...
                {
                    if (spell != null && spell.Timer != null && spell.GetType() == type && spell.Target == target)
                        return spell;
                }
            }
        ...
        public static SkillMasterySpell GetSpell(Mobile from, Type type)
        {
            ...
            {
                return m_Table[from].FirstOrDefault(spell => spell != null && spell.Timer != null && spell.GetType() == type);
            }
        ...
        public static TSpell GetSpell<TSpell>(Mobile m) where TSpell : SkillMasterySpell
        {
            ...
            {
                return m_Table[m].FirstOrDefault(sms => sms.Timer != null && sms.GetType() == typeof(TSpell)) as TSpell;
            }
        ...
        public static TSpell GetSpell<TSpell>(Mobile caster, Mobile target) where TSpell : SkillMasterySpell
        {
            ...
            {
                return m_Table[caster].FirstOrDefault(sms => sms.Timer != null && sms.GetType() == typeof(TSpell) && sms.Target == target) as TSpell;
            }
        ...
        public static TSpell GetSpell<TSpell>(Func<SkillMasterySpell, bool> predicate) where TSpell : SkillMasterySpell
        {
            foreach (var pair in m_Table)
            {
                ...
                {
                    if (spell is TSpell result && spell.Timer != null && predicate != null && predicate(spell)) { return result; }
                }
            }
            return null;
        }
        ...
        public static void CheckTable(Mobile m)
        {
            ...
            // Main-loop-only collection access.
            {
                if (m_Table.ContainsKey(m))
                {
        ...
        public static IEnumerable<SkillMasterySpell> EnumerateSpells(Mobile from, Type t = null)
        {
            ...
            // Main-loop-only collection access.
            {
                e = list.Where(s => s.Timer != null && (s.GetType() == t || t == null));
            }
		...
		public static void OnDamage(Mobile victim, Mobile damager, DamageType type, ref int damage)
		{
            ...
            using var targetEffects = Server.Collections.PooledRefList<SkillMasterySpell>.Create();
            foreach (var pair in m_Table)
            {
                foreach (var effect in pair.Value) { if (effect.Timer != null && effect.Target == victim) { targetEffects.Add(effect); } }
            }
            foreach (SkillMasterySpell sp in targetEffects)
            {
		...
		public static bool HasMastery(Mobile mobile, SkillName name)
		{
            ...
            return MasteryInfo.HasLearned(mobile, name);
		}

        public static bool SetActiveMastery(Mobile mobile, SkillName name)
        {
            return BookOfMasteries.Select(mobile, name);
        }
        ...
        public static int GetAttributeBonus(Mobile m, AosAttribute attr)
        {
            ...
            switch (attr)
            {
                case AosAttribute.AttackChance:
                    ...
                    spell = SkillMasterySpell.GetHarmfulSpell(m, typeof(TribulationSpell));
                    if (spell != null) { value -= spell.PropertyBonus(); }
        ...
        public class MasteryTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object o)
            {
                ...
                {
                    if (o is IPoint3D point) { SpellHelper.Turn(from, point); }
                    Owner.OnTarget(o);
                }
            }
		...
		public class UpkeepTimer : Timer
		{
			...
			public void Release() { Stop(); m_Spell = null; }
            ...
            e.Mobile.BeginTarget(-1, false, TargetFlags.None, (mobile, targeted) =>
                {
                    if (targeted is Mobile)
                    {
                        ...
                        {
                            if (Array.IndexOf(MasteryInfo.Skills, sk.SkillName) >= 0)
                            {
                                count++;
                                MasteryInfo.LearnMastery(m, sk.SkillName, 3);
                            }
                        }

                        e.Mobile.SendMessage($"They have learned {count} masteries!");
                    }
                });
        }
