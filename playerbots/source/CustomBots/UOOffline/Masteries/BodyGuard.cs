    ...
    public class BodyGuardSpell : SkillMasterySpell
    {
                ...
                Caster.BeginTarget(8, false, Server.Targeting.TargetFlags.None, (m, o) =>
                    {
                        ...
                        {
                            if (o is IPoint3D point) { SpellHelper.Turn(Caster, point); }
                            OnTarget(o);
                        }
                    });
            }
		...
		public void AcceptBodyGuard(Mobile toGuard)
		{
			...
			if(CheckBSequence(toGuard))
			{
                ...
                MasteryBuffInfo.AddBuff(Caster, new MasteryBuffInfo(BuffIcon.Bodyguard, 1155924, 1156061, TimeSpan.FromSeconds(90), Caster, String.Format("{0}\t{1}\t{2}\t{3}", Caster.Name, ((int)(_Block + 5)).ToString(), toGuard.Name, ((int)_Block).ToString())));
                MasteryBuffInfo.AddBuff(toGuard, new MasteryBuffInfo(BuffIcon.Bodyguard, 1155924, 1156061, TimeSpan.FromSeconds(90), toGuard, String.Format("{0}\t{1}\t{2}\t{3}", Caster.Name, ((int)(_Block + 5)).ToString(), toGuard.Name, ((int)_Block).ToString())));
        ...
        public override void EndEffects()
        {
            if (Target != null)
            {
                ...
                MasteryBuffInfo.RemoveBuff(Target, BuffIcon.Bodyguard);
            }

            Caster.SendLocalizedMessage(1156103); // Bodyguard has expired.
            MasteryBuffInfo.RemoveBuff(Caster, BuffIcon.Bodyguard);
        }
