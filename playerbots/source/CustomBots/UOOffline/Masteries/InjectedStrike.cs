...
namespace Server.Spells.SkillMasteries
{
    public class InjectedStrikeSpell : SkillMasterySpell
    {
			...
			if(CheckWeapon())
			{
                ...
                else if (!HasSpell(Caster, this.GetType()))
                {
                    if (CheckSequence())
                    {
                        ...
                        MasteryBuffInfo.AddBuff(Caster, new MasteryBuffInfo(BuffIcon.InjectedStrike, 1155927, 1156163, String.Format("{0}\t{1}", bonus.ToString(), (bonus / 2).ToString())));
        ...
        {
            MasteryBuffInfo.RemoveBuff(Caster, BuffIcon.InjectedStrike);
        }
        ...
        public override void OnHit(Mobile defender, ref int damage)
        {
            ...
            ResistanceMod mod = new ResistanceMod(ResistanceType.Poison, "Mastery InjectedStrike", -malus);
            defender.AddResistanceMod(mod);

            // ~2_NAME~ reduces your poison resistance by ~1_VAL~.
            MasteryBuffInfo.AddBuff(defender, new MasteryBuffInfo(BuffIcon.InjectedStrikeDebuff, 1155927, 1156133, TimeSpan.FromSeconds(7), defender, String.Format("{0}\t{1}", malus, Caster.Name)));
