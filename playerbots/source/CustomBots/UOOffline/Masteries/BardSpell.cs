...
namespace Server.Spells.SkillMasteries
{
	public abstract class BardSpell : SkillMasterySpell
	{
		...
		{
            if (useSkill == MasteryProgress.Current(pm))
                return 10;

            if (MasteryProgress.Current(pm) == SkillName.Provocation
                || MasteryProgress.Current(pm) == SkillName.Discordance
                || MasteryProgress.Current(pm) == SkillName.Peacemaking)
                return 5;
