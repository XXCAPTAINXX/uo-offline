...
namespace Server.Spells.SkillMasteries
{
    ...
    public class CombatTrainingSpell : SkillMasterySpell
    {
        ...
        {
            if (Caster is PlayerMobile player && (player.AllFollowers == null || player.AllFollowers.Count == 0))
            {
                Caster.SendLocalizedMessage(1156112); // This ability requires you to have pets.
                return false;
            }

            var active = GetSpell<CombatTrainingSpell>(Caster);
            if (active != null) { active.Expire(); return false; }
        ...
        public void OnSelected(TrainingType type, Mobile target)
        {
            ...
            MasteryBuffInfo.AddBuff(Caster, new MasteryBuffInfo(BuffIcon.CombatTraining, 1155933, 1156107, String.Format("{0}\t{1}\t{2}", SpellType.ToString(), Target.Name, ((int)ScaleUpkeep()).ToString())));
        ...
        {
            MasteryBuffInfo.RemoveBuff(Caster, BuffIcon.CombatTraining);
                ...
                if (spell != null)
                {
                    ...
                    switch (spell.SpellType)
                    {
                        ...
                        case TrainingType.ConsumeDamage:
                            if (spell.Phase < 2)
                            {
                                attacker.NetState.SendDamage(defender.Serial, damage);
                                damage = 0;
                            }
                            break;
