...
namespace Server.Spells.SkillMasteries
{
    ...
    public class MasteryInfo
    {
        ...
        public static void Configure()
        {
            ...
            {
                if (SpellRegistry.GetRegistryNumber(entry.SpellType) < 0) { SpellRegistry.Register(entry.SpellID, entry.SpellType); }
            }
        ...
        {
            return GetMasteryLevel(m, skill) > 0;
        }

        public static bool HasLearned(Mobile m, SkillName skill, int volume)
        {
            return GetMasteryLevel(m, skill) >= volume;
        }
        ...
        public static bool HasLearned(Mobile m, Type spell, SkillName skill)
        {
            ...
            return info != null && GetMasteryLevel(m, info.MasterySkill) > 0;
        }

        public static int GetMasteryLevel(Mobile m, SkillName name)
        {
            if (m is Server.UOOffline.HavenCompanion companion && companion.Role == Server.UOOffline.HavenCompanionRole.Bard && name is SkillName.Provocation or SkillName.Peacemaking or SkillName.Discordance)
            { var skill = Math.Min(m.Skills[name].Value, m.Skills.Musicianship.Value); return skill < 90 ? 0 : Math.Clamp(1 + (int)((skill - 90) / 10), 1, 3); }
            return MasteryProgress.Find(m)?.Level(name) ?? 0;
        }
        ...
        public static bool LearnMastery(Mobile m, SkillName skill, int volume)
        {
            ...
            {
                return MasteryProgress.Get(m).Learn(skill, volume);
                return true;
            }
        ...
        {
            return spellID == 714 || spellID == 715 || spellID == 717 || spellID == 732 || spellID == 738 || spellID == 741 || spellID == 744;
        }

        public static void OnMasteryChanged(Mobile m, SkillName oldMastery)
        {
            PassiveSpell passive = GetActivePassive(m);
            SkillName newMastery = MasteryProgress.Current(m);
            ...
            if (passive != PassiveSpell.None && passive != PassiveSpell.AnticipateHit)
            {
                switch (passive)
                {
                    case PassiveSpell.EnchantedSummoning:
                        MasteryBuffInfo.AddBuff(m, new MasteryBuffInfo(BuffIcon.EnchantedSummoning, 1155904, 1156090, String.Format("{0}\t{0}", EnchantedSummoningBonus(m).ToString()), true)); // +~1_STAMINA~ Stamina Regeneration and +~2_HP~% Hit Points for summoned pets.<br>Increased difficulty for summoned pets to be dispelled.
                        break;
                    case PassiveSpell.Intuition:
                        MasteryBuffInfo.AddBuff(m, new MasteryBuffInfo(BuffIcon.Intuition, 1155907, 1156089, IntuitionBonus(m).ToString(), true)); // Mana Increase ~1_VAL~
                        break;
                    case PassiveSpell.SavingThrow:
                        {
                            ...
                            if (GetMasteryLevel(m, newMastery) >= 3) { m.AddStatMod(new StatMod(StatType.Str, "SavingThrow_Str", 5, TimeSpan.Zero)); }
                            MasteryBuffInfo.AddBuff(m, new MasteryBuffInfo(BuffIcon.SavingThrow, 1156031, 1156032, args, true)); // Provides a chance to block disarm attempts based on Mastery level, weapon skill level and tactics skill level.
                        }
                        break;
                    case PassiveSpell.Potency:
                        MasteryBuffInfo.AddBuff(m, new MasteryBuffInfo(BuffIcon.Potency, 1155928, 1156195, NonPoisonConsumeChance(m).ToString(), true)); // ~1_VAL~% chance to not consume poison charges when using infecting strike or injected strike.
                        break;
                    case PassiveSpell.Knockout:
                        MasteryBuffInfo.AddBuff(m, new MasteryBuffInfo(BuffIcon.Knockout, 1155931, 1156030, String.Format("{0}\t{1}", GetKnockoutModifier(m).ToString(), GetKnockoutModifier(m, true).ToString(), true))); // Wrestling Damage Bonus:<br>+~1_VAL~% PvM<br>+~2_VAL~% PvP
                        break;
                    case PassiveSpell.Boarding:
                        MasteryBuffInfo.AddBuff(m, new MasteryBuffInfo(BuffIcon.Boarding, 1155934, 1156194, BoardingSlotIncrease(m).ToString(), true)); // Your number of stable slots has been increased by ~1_VAL~.
                        break;
                }
        ...
        public static PassiveSpell GetActivePassive(Mobile m)
        {
            ...
            SkillName mastery = MasteryProgress.Current(m);
        ...
        public static bool IsActivePassive(Mobile m, PassiveSpell spell)
        {
            if (spell == PassiveSpell.AnticipateHit)
                return MasteryProgress.Current(m) == SkillName.Bushido;
        ...
        {
            MasteryBuffInfo.RemoveBuff(m, BuffIcon.EnchantedSummoning);
            MasteryBuffInfo.RemoveBuff(m, BuffIcon.AnticipateHit);
            MasteryBuffInfo.RemoveBuff(m, BuffIcon.Intuition);
            MasteryBuffInfo.RemoveBuff(m, BuffIcon.SavingThrow);
            MasteryBuffInfo.RemoveBuff(m, BuffIcon.Potency);
            MasteryBuffInfo.RemoveBuff(m, BuffIcon.Knockout);
            MasteryBuffInfo.RemoveBuff(m, BuffIcon.Boarding);
        }

        [ModernUO.CodeGeneratedEvents.OnEvent(nameof(PlayerMobile.PlayerLoginEvent))]
        public static void OnLogin(PlayerMobile m)
        {


            if ((int)MasteryProgress.Current(m) > 0)
                OnMasteryChanged(m, MasteryProgress.Current(m));
        }
        ...
        public static int EnchantedSummoningBonus(Mobile m)
        {
            ...
            {
                SkillName sk = MasteryProgress.Current(m);
        ...
        public static int SavingThrowChance(Mobile m, AosAttribute attr)
        {
            ...
            {
                int level = GetMasteryLevel(m, MasteryProgress.Current(m));
