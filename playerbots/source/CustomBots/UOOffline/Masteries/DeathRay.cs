...
namespace Server.Spells.SkillMasteries
{
	public class DeathRaySpell : SkillMasterySpell
	{
            ...
            if (m != null)
            {
                ...
                else if (CheckHSequence(m))
                {
                    ...
                    else
                    {
                        ...
                        _Mod = new ResistanceMod(ResistanceType.Energy, "Mastery DeathRay", -mod);
                        m.AddResistanceMod(_Mod);

                        MasteryBuffInfo.AddBuff(Caster, new MasteryBuffInfo(BuffIcon.DeathRay, 1155896, 1156085, String.Format("{0}\t{1}", ((int)damage).ToString(), m.Name))); // Deals ~2_DAMAGE~ to ~1_NAME~ every 3 seconds while in range. Preforming any action will end spell.
                        MasteryBuffInfo.AddBuff(m, new MasteryBuffInfo(BuffIcon.DeathRayDebuff, 1155896, 1156086, mod.ToString())); // Energy Resist Debuff: ~1_VAL~%
        ...
        public override void EndEffects()
        {
            ...
            MasteryBuffInfo.RemoveBuff(Caster, BuffIcon.DeathRay);

            if(Target != null)
                MasteryBuffInfo.RemoveBuff(Target, BuffIcon.DeathRayDebuff);
        }
        ...
        public override bool OnTick()
        {
            ...
            else
            {
                ...
                int sdiBonus = MasteryTargeting.SpellDamageBonus(Caster, Target, CastSkill, Caster.Player && Target.Player);
