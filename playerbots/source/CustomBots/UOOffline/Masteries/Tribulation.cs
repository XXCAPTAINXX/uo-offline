	...
	public class TribulationSpell : BardSpell
	{
		...
		public void OnTarget( Mobile m )
		{
            ...
            else if (CheckHSequence(m))
            {
                ...
                MasteryBuffInfo.AddBuff(m, new MasteryBuffInfo(BuffIcon.TribulationTarget, 1115740, 1115742, String.Format("{0}\t{1}\t{2}", m_PropertyBonus, m_PropertyBonus, (int)m_DamageChance)));

                // Target: ~1_val~ <br> Damage Factor: ~2_val~% <br> Damage Chance: ~3_val~%
                MasteryBuffInfo.AddBuff(Caster, new MasteryBuffInfo(BuffIcon.TribulationCaster, 1115740, 1151388, String.Format("{0}\t{1}\t{2}", m.Name, (int)m_DamageFactor, (int)m_DamageChance)));
        ...
        {
            MasteryBuffInfo.RemoveBuff(Target, BuffIcon.TribulationTarget);
            MasteryBuffInfo.RemoveBuff(Caster, BuffIcon.TribulationCaster);
        }

        public override void OnTargetDamaged(Mobile attacker, Mobile victim, DamageType type, ref int damageTaken)
		{
			if(m_NextDamage > Core.Now)
				return;

            if (m_DamageChance / 100 > Utility.RandomDouble())
			{
                m_NextDamage = Core.Now + TimeSpan.FromSeconds(1);
                ...
                AOS.Damage(victim, Caster, damage, false, 0, 0, 0, 0, 0, 0, 100);
