        ...
        protected override void OnTarget(object o)
        {
            ...
            if (m != null)
            {
                if (CheckHSequence(m))
                {
                    ...
                    int sdiBonus = MasteryTargeting.SpellDamageBonus(Caster, m, CastSkill, m is Mobile ? Caster.Player && ((Mobile)m).Player : false);
                    ...
                    if (target is Mobile && !CheckResisted((Mobile)target) && ((Mobile)target).NetState != null)
                    {
                        ...
                        if (!TransformationSpellHelper.UnderTransformation(mob, typeof(AnimalForm)))
                            mob.NetState.SendSpeedControl(SpeedControlSetting.Walk);
                        ...
                        Server.Timer.DelayCall(TimeSpan.FromSeconds(skill / 60), () =>
                            {
                                if (!TransformationSpellHelper.UnderTransformation(mob, typeof(AnimalForm)) &&
                                    ...
                                    mob.NetState.SendSpeedControl(SpeedControlSetting.Disable);
                            });
                    }
                }
            }
