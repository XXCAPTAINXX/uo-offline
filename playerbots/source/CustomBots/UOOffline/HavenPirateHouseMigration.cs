...
public partial class HavenPirateHeadquarters
{
    internal static HavenPirateHeadquarters Install(Mobile owner)
    {
        ...
        foreach (var ready in Registry) { if (!ready.Deleted && ready.Owner == owner) { ready.RebuildCompound(); ready.RefineFloorAccess(); ready.Estate?.MoveTrialToNorthwest(); ready.Estate?.EnsureHomePatrol(); return ready; } }
    ...
    internal static HavenPirateHeadquarters Replace(HavenGuildCastle old)
    {
        ...
        try
        {
            ...
            house.RebuildCompound(); house.RefineFloorAccess(); house.Estate?.MoveTrialToNorthwest(); house.Estate?.EnsureHomePatrol();
