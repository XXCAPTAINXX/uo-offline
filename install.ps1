...
function InstallUORespawn {
  ...
  if (Test-Path $spawnSettings) {
    ...
    $settingsText = $settingsText -replace '(?m)^ENABLE_VENDOR_SPAWN,.*
...
# accidentally deleted. That is exactly how the GUI reached "Build the
# server" with no BuildModernUO function. Fail immediately instead.
# ---------------------------------------------------------------------------
...
, 'ENABLE_VENDOR_SPAWN,False'
    $settingsText = $settingsText -replace '(?m)^ENABLE_VENDOR_NIGHT,.*
  New-Item -ItemType Directory -Force -Path $licenseDir | Out-Null
  Invoke-WebRequest -Uri $UORespawnLicenseUrl -OutFile (Join-Path $licenseDir "UORespawn-MIT.txt") -Headers @{ "User-Agent" = "uo-offline-installer" }
...
, 'ENABLE_VENDOR_NIGHT,False'
    $settingsText = $settingsText -replace '(?m)^ENABLE_VENDOR_EXTRA,.*
  New-Item -ItemType Directory -Force -Path $licenseDir | Out-Null
  Invoke-WebRequest -Uri $UORespawnLicenseUrl -OutFile (Join-Path $licenseDir "UORespawn-MIT.txt") -Headers @{ "User-Agent" = "uo-offline-installer" }
