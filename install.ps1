...
function InstallUORespawn {
  ...
  # The dedicated newbie dungeon is a controlled training room. UORespawn
  # must not inject its normal dungeon ecology while a player is inside it.
  ...
  # ModernUO's native Vendors.json files own fixed town services. Keep
  # UORespawn focused on ecology so banks/shops/stables are not duplicated.
  ...
  if (Test-Path $spawnSettings) {
    ...
    $settingsText = $settingsText -replace '(?m)^ENABLE_VENDOR_SPAWN,.*$', 'ENABLE_VENDOR_SPAWN,False'
    $settingsText = $settingsText -replace '(?m)^ENABLE_VENDOR_NIGHT,.*$', 'ENABLE_VENDOR_NIGHT,False'
    $settingsText = $settingsText -replace '(?m)^ENABLE_VENDOR_EXTRA,.*$', 'ENABLE_VENDOR_EXTRA,False'
...
function InstallTazUO {
  ...
  if (Test-Path $known) {
    ...
    if ($knownExe -and (Test-Path $knownExe)) {
      Ok "TazUO already present: $knownExe"
      return
    }
  }
...
# ---------------------------------------------------------------------------
# Installer structural self-check.
#
# PowerShell can parse a script even when a function referenced later was
# accidentally deleted. Fail immediately if any GUI/console install step is
# missing its implementation.
# ---------------------------------------------------------------------------
function Assert-InstallerFunctions {
  $required = @(
    "Preflight",
    "BootstrapGit",
    "BootstrapDotnet",
  ...
  if ($missing.Count -gt 0) {
    throw "Installer is incomplete; missing function(s): $($missing -join ', ')"
  }
}
