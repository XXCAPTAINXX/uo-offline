...
namespace Server.HavenPrototype
{
 public class HavenLegendaryPetSkills:Item
 {
  ...
  private bool _expanded; Timer _innateTimer; int _nextInnate;
  ...
  private void Repair(BaseCreature pet){foreach(var name in _boosted){var skill=pet.Skills[name];skill.Cap=Math.Max(skill.Cap,Math.Max(125,skill.Base));skill.Base=Math.Max(125,skill.Base);}HavenLegendaryInnates.Repair(pet,_boosted);StartInnates();pet.InvalidateProperties();}
