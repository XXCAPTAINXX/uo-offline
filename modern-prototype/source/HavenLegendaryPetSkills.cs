...
namespace Server.HavenPrototype
{
 public class HavenLegendaryPetSkills:Item
 {
  ...
  private bool _expanded,_tamed; Timer _innateTimer; int _nextInnate;
  void StartInnates(){if(_innateTimer==null)_innateTimer=Timer.DelayCall(TimeSpan.FromSeconds(2),TimeSpan.FromSeconds(6),()=>{var pet=RootParent as BaseCreature;if(!HavenPreview.Enabled||!HavenLegendaryInnates.Active(pet))return;var choices=HavenLegendaryInnates.Granted(_boosted).Where(n=>HavenLegendaryInnates.Actions.Contains(n)).ToArray();for(int i=0;i<choices.Length;i++){var choice=choices[_nextInnate%choices.Length];_nextInnate=(_nextInnate+1)%choices.Length;if(HavenLegendaryInnates.TryUse(pet,choice))break;}});}
  public override void OnDelete(){if(_innateTimer!=null)_innateTimer.Stop();base.OnDelete();}
  ...
  public static bool Protects(BaseCreature pet,SkillName name){var record=pet.Backpack==null?null:pet.Backpack.FindItemByType(typeof(HavenLegendaryPetSkills),true) as HavenLegendaryPetSkills;return HavenPreview.Enabled&&HavenPetDefenses.Tier(pet)==3&&record!=null&&HavenLegendaryInnates.RequiredSkills(record._boosted).Contains(name);}
  public static HavenLegendaryPetSkills Find(BaseCreature pet){return pet==null||pet.Backpack==null?null:pet.Backpack.FindItemByType(typeof(HavenLegendaryPetSkills),true) as HavenLegendaryPetSkills;}
  public void MarkTamed(){_tamed=true;}
  private void Repair(BaseCreature pet){if(!_tamed&&!pet.Controlled&&pet.Owners.Count==0){foreach(var name in _boosted){var skill=pet.Skills[name];skill.Cap=Math.Max(skill.Cap,Math.Max(125,skill.Base));skill.Base=Math.Max(125,skill.Base);}HavenLegendaryInnates.Repair(pet,_boosted);}StartInnates();pet.InvalidateProperties();}
  ...
  public override void Serialize(GenericWriter w){base.Serialize(w);w.Write(2);w.Write(_boosted.Count);foreach(var name in _boosted)w.Write((int)name);w.Write(_expanded);w.Write(_tamed);}
  public override void Deserialize(GenericReader r){base.Deserialize(r);int version=r.ReadInt();int count=r.ReadInt();if(count<0||count>5)throw new InvalidOperationException("Invalid legendary skills");_boosted.Clear();for(int i=0;i<count;i++)_boosted.Add((SkillName)r.ReadInt());_expanded=version>=1 && r.ReadBool();_tamed=version>=2 && r.ReadBool();}
 }
}
