...
namespace Server.HavenPrototype {
    public static class HavenRecovery {
        ...
        public static int ResurrectPets(Mobile from){if(!CanUse(from))return 0;int count=0;var near=from.GetMobilesInRange(12);try{foreach(Mobile m in near){var pet=m as BaseCreature;if(pet!=null && pet.IsDeadPet && pet.ControlMaster==from && from.InLOS(pet) && pet.Map.CanFit(pet.Location,16,false,false)){pet.ResurrectPet();count++;}}}finally{near.Free();}return count;}
        public static bool RecoverCompanion(Mobile from){if(!CanUse(from))return false;var c=World.Mobiles.Values.OfType<HavenCompanion>().FirstOrDefault(x=>!x.Deleted && x.BoundOwner==from && x.ControlMaster==from && x.IsDeadPet && !x.IsStabled && !x.OnMission);if(c==null)return false;c.MoveToWorld(from.Location,from.Map);c.ResurrectPet();c.SetOrder(from,OrderType.Follow);c.AIObject.Activate();return true;}
