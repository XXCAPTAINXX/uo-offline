...
namespace Server.HavenPrototype {
    public static class HavenRecovery {
        ...
        public static bool RecallCorpse(Mobile from){return RecallCorpses(from)>0;}
        public static int RecallCorpses(Mobile from){
            if(!CanUse(from)||!from.Map.CanFit(from.Location,16,false,false))return 0;
            var corpses=World.Items.Values.OfType<Corpse>().Where(c=>!c.Deleted&&c.Owner==from&&c.Parent==null&&c.Map!=null&&c.Map!=Map.Internal).ToArray();
            foreach(var corpse in corpses){corpse.MoveToWorld(from.Location,from.Map);corpse.BeginDecay(TimeSpan.FromMinutes(10));}
