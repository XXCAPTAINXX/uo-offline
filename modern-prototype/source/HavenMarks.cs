...
namespace Server.HavenPrototype
{
    ...
    public class HavenMarksGump:HavenStoneGump
    {
        ...
        public HavenMarksGump(Mobile owner,int selected):base(45,45) {
            ...
            for(int i=0;i<HavenMarks.Names.Length;i++) {var item=HavenMarks.CreateReward(i);_holder.DropItem(item);if(i==_selected)_preview=item;ItemButton(owner,item,20,112+i*39,250,100+i,HavenMarks.Names[i]);}
            ...
            ItemButton(owner,_preview,290,321,335,1,"Buy selected reward");
            if(HavenMarks.CanClaimTestAllowance(owner)) {
                FlatButton(20,370,410,2,"Claim 100 test Marks (once per account)");
            } else AddLabel(20,370,0,"Test allowance claimed. Missions earn more Marks.");
            FlatButton(545,370,85,0,"Close");
        }
