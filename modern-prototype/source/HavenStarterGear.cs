...
namespace Server.HavenPrototype {
 ...
 public static class HavenStarterGear {
  ...
  public static void Ensure(){var existing=World.Items.Values.OfType<HavenStarterGearStone>().FirstOrDefault(x=>!x.Deleted);if(existing!=null){existing.ItemID=0xED4;existing.Hue=0;return;}Point3D p;if(HavenPreview.FindLanding(new HavenPreview.Destination("Evolving starter gear",Map.Trammel,3500,2572,14),out p))new HavenStarterGearStone().MoveToWorld(p,Map.Trammel);}
