...
namespace Server.HavenPrototype {
 ...
 public static class HavenStarterGear {
  ...
  public static void Properties(IHavenStarterGear g,ObjectPropertyList list){list.Add(g.Kind==4?"Robe upgrade tier: "+g.Progress.Tier+"/4":HavenLevelProperties.Line(Level(g),g.Progress.Experience,Limit(g)));if(g.Kind!=4){int level=Level(g);int next=g.Kind==6?25*level*level:g.Kind>=7?100*level:5*level*level+25*level;HavenLevelProperties.Progress(list,level,g.Progress.Experience,next);list.Add("Wear to gain XP from credited hostile monster kills.");if(g is BaseWeapon){list.Add("Also gains XP on hits against eligible hostile monsters.");if(level<20)list.Add("Next level: "+(level+1)+"% damage increase; mana leech at least "+Math.Max(30,20+level*80/19)+"%");}else if(g.Kind==5)list.Add("Also gains XP from successful spell sequences.");}if(g.Progress.Owner!=null)list.Add("Bound to: "+g.Progress.Owner.Name);}
 ...
 public class HavenStarterGearGump:HavenStoneGump {
  ...
  private void Button(int x,int y,int id,string label){FlatButton(x,y,id==0?100:id==3?230:260,id,label);}
