...
namespace Server.HavenPrototype {
 public class HavenOfflineMissionPlan:Item {
  ...
  public int RouteMask=(1<<((int)CompanionMission.DoomRecon+1))-1;
  public override bool IsVirtualItem {get{return true;}}
  public bool Includes(CompanionMission kind){return kind>=0&&kind<=CompanionMission.DoomRecon&&(RouteMask&(1<<(int)kind))!=0;}
  public bool ToggleRoute(Mobile owner,CompanionMission kind){if(Companion==null||!Companion.IsOwner(owner)||kind<0||kind>CompanionMission.DoomRecon)return false;RouteMask^=1<<(int)kind;return true;}

  public bool CanRun(Mobile owner){return Enabled&&HavenAfkMissions.IsAway(owner,AutoAfk);}
  ...
  public void Tick(){var c=Companion;var owner=c?.BoundOwner;if(c==null||c.Deleted||owner==null||owner.Deleted){Delete();return;}if(Parent!=c.Backpack){Enabled=false;Status="Plan must remain in companion pack.";return;}
   ...
   for(int i=0;i<(Rotate?count:1);i++){var kind=Rotate?(CompanionMission)((Cursor+i)%count):Focus;if(Rotate&&!Includes(kind))continue;reason=c.MissionStartError(owner,Minutes,kind,true);if(reason!=null)continue;if(c.StartOfflineMission(Minutes,kind)){ActiveTrip=true;if(Rotate)Cursor=((int)kind+1)%count;Status="Running "+CompanionActivityGump.MissionName(kind)+" for "+Minutes+" minutes.";return;}reason="Reward storage is full; collect pending rewards.";}
   Status=reason??"No selected eligible mission. Choose rotation routes.";
  }
  public override void Serialize(GenericWriter w){base.Serialize(w);w.Write(2);w.Write(Companion);w.Write(Enabled);w.Write(Rotate);w.Write(RecallOnLogin);w.Write(Minutes);w.Write(Cursor);w.Write((int)Focus);w.Write(ActiveTrip);w.Write(AutoAfk);w.Write(RouteMask);}
  public override void Deserialize(GenericReader r){base.Deserialize(r);int version=r.ReadInt();Companion=r.ReadMobile() as HavenCompanion;Enabled=r.ReadBool();Rotate=r.ReadBool();RecallOnLogin=r.ReadBool();Minutes=r.ReadInt();Cursor=r.ReadInt();Focus=(CompanionMission)r.ReadInt();ActiveTrip=r.ReadBool();if(version>=1)r.ReadBool();RouteMask=version>=2?r.ReadInt():(1<<((int)CompanionMission.DoomRecon+1))-1;RouteMask&=(1<<((int)CompanionMission.DoomRecon+1))-1;AutoAfk=false;if(Minutes!=5&&Minutes!=15&&Minutes!=30&&Minutes!=60){Minutes=5;Enabled=false;}if(Focus<0||Focus>CompanionMission.DoomRecon){Focus=0;Enabled=false;}Cursor=Math.Max(0,Cursor)%((int)CompanionMission.DoomRecon+1);Schedule();}
 ...
 public class HavenOfflineMissionGump:HavenMenuGump {
  ...
  public HavenOfflineMissionGump(HavenOfflineMissionPlan p,CompanionMission? candidate=null,int duration=5):base(80,80){_plan=p;_candidate=candidate??p.Focus;_duration=candidate.HasValue?duration:p.Minutes;AddBackground(0,0,560,630,0xA28);Text(24,20,510,30,"Offline / AFK companion missions");Text(24,59,510,50,"Default: "+CompanionActivityGump.MissionName(p.Focus)+" � "+p.Minutes+" minutes.<BR>Choose and save your route and duration below.");Button(24,121,1,p.Enabled?"Offline missions: ON":"Offline missions: OFF");Button(24,166,2,p.Rotate?"Route: cycle selected missions":"Route: repeat saved default");Button(24,211,3,p.RecallOnLogin?"On return: recall early":"On return: finish current trip");Text(24,252,508,102,"Trips keep their normal skill and combat requirements. Rewards arrive loose in the companion backpack; overflow is retained. No backdated extra trips while the server is stopped.<BR>"+p.Status);Text(24,352,510,40,"Selected: "+CompanionActivityGump.MissionName(_candidate.Value));Button(24,394,6,"Choose mission");int[] choices={5,15,30,60};for(int i=0;i<4;i++){int value=choices[i];Button(24+i*126,432,20+i,(_duration==value?"[":"")+value+" min"+(_duration==value?"]":""));}Button(24,475,5,"Save mission and duration");Button(24,513,7,HavenAfkMissions.IsManual(p.Companion.BoundOwner)?"AFK mode: ON (click to return)":"Start AFK mode now");Button(24,550,9,"Choose rotation routes");Button(24,589,4,"Refresh");Button(360,589,0,"Back");}
