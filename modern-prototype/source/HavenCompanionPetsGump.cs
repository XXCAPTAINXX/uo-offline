...
namespace Server.HavenPrototype
{
 public class HavenCompanionPetsGump:HavenPetMenuGump
 {
  ...
  public static void Show(Mobile p,HavenCompanion c,int page=0){if(c!=null&&c.ShowAwayTimer(p))return;if(!CanUse(p,c)){p.SendMessage("Bring your companion within 12 tiles to manage assigned pets.");return;}p.CloseGump(typeof(HavenCompanionPetsGump));p.SendGump(new HavenCompanionPetsGump(c,page));}
  ...
  void Button(int x,int y,int id,string label){FlatButton(x,y,id>=100?130:id<=3&&id!=1?110:190,id,label);}
  public override void OnResponse(NetState state,RelayInfo info){var p=state.Mobile;int id=info.ButtonID;if(id==0)return;if(_companion.ShowAwayTimer(p))return;if(!CanUse(p,_companion))return;if(id==1){p.Target=new AssignTarget(_companion);p.SendMessage("Target your living, dismounted pet within three tiles of the companion.");return;}if(id==4){_companion.RequestTamingAssist(p);return;}if(id==2||id==3){Show(p,_companion,_page+(id==2?-1:1));return;}int index=(id-100)/2;if(id>=100&&index<_pets.Length){var item=_pets[index];if(!item.Deleted&&item.Owner==p&&item.Companion==_companion&&item.Pet.ControlMaster==_companion){if((id-100)%2==0){item.AutoMount=!item.AutoMount;item.Tick();}else if(!item.ClaimBack(p))p.SendMessage("Reclaim failed: check pet health and space in the companion pack.");}}Show(p,_companion,_page);}
