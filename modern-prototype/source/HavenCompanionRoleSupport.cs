...
namespace Server.HavenPrototype {
 public partial class HavenCompanion {
  ...
  internal bool CanSupportRole(Mobile target){return !Deleted&&Alive&&!IsDeadPet&&!OnMission&&!IsStabled&&target!=null&&!target.Deleted&&target.Alive&&Map!=null&&Map!=Map.Internal&&target.Map==Map&&InRange(target,SupportRange)&&InLOS(target)&&(target==this||target==BoundOwner||(target.Player&&SupportParty.Get(BoundOwner)?.Contains(target)==true));}
  ...
  internal SkillName ChooseBardMastery(Mobile enemy){if(Skills.Musicianship.Value<90)return SkillName.Alchemy;if(TamingAssistActive)return Skills.Peacemaking.Value>=90?SkillName.Peacemaking:SkillName.Alchemy;if(Skills.Peacemaking.Value>=90&&BoundOwner!=null&&BoundOwner.Hits<BoundOwner.HitsMax*0.7)return SkillName.Peacemaking;if(Skills.Peacemaking.Value>=90)return SkillName.Peacemaking;if(!TamingAssistActive&&Skills.Discordance.Value>=90&&BardEnemy(enemy)&&enemy.HitsMax>=500)return SkillName.Discordance;if(Skills.Provocation.Value>=90)return SkillName.Provocation;return Skills.Peacemaking.Value>=90?SkillName.Peacemaking:SkillName.Alchemy;}
  bool HasSong<T>() where T:SkillMasterySpell {return SkillMasterySpell.GetSpells(this)?.Any(x=>x is T&&x.Timer!=null)==true;}
  internal bool ThinkBardMasteries(){if(TamingAssistActive)return false;if(Role!=CompanionRole.Bard||!CanSupportRole(BoundOwner)||Spell!=null||Target!=null||DateTime.UtcNow<_nextMastery||Mana<30)return false;_nextMastery=DateTime.UtcNow.AddSeconds(3);var enemy=(Combatant??BoundOwner.Combatant) as Mobile;var desired=ChooseBardMastery(enemy);if(desired==SkillName.Alchemy)return false;
   if(_bardMastery!=desired&&(DateTime.UtcNow>=_nextMasterySwitch||_bardMastery==SkillName.Alchemy||(TamingAssistActive&&_bardMastery!=SkillName.Peacemaking))){ClearBardMasteries();_bardMastery=desired;_nextMasterySwitch=DateTime.UtcNow.AddSeconds(30);foreach(var p in _songRecipients)if(!p.Deleted)RemoveSong(p);_songRecipients.Clear();}
