...
namespace Server.HavenPrototype {
 ...
 public class HavenWardenGump:HavenMenuGump {
  ...
  public override void OnResponse(NetState state,RelayInfo info){if(info.ButtonID!=1||_post.Deleted)return;var p=state.Mobile;Point3D landing;if(!HavenPreview.CanTravel(p)||!HavenPreview.FindLanding(new HavenPreview.Destination("Warden",_post.Map,_post.X,_post.Y,_post.Z),out landing)){p.SendMessage("Travel unavailable; leave combat and wait for recent combat to expire.");return;}BaseCreature.TeleportPets(p,landing,_post.Map);p.MoveToWorld(landing,_post.Map);}
 }
}
