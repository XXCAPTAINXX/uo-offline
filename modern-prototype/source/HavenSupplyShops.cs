...
namespace Server.HavenPrototype {
 ...
 public static class HavenSupplyShops {
  ...
  static List<HavenShopEntry> Rewards(){var list=new List<HavenShopEntry>();string[] areaNames={"Stormblade","Cindermaul","Frostwake Bow"};string[] elements={"energy","fire","cold"};for(int a=0;a<3;a++){int index=a;list.Add(new HavenShopEntry(areaNames[a],0,()=>HavenAreaWeapons.Create(index),"Leveling "+elements[a]+" area weapon. Area-hit chance grows from 20% to 77% at level20. Equip and defeat hostile monsters for XP; native area targeting and attribute caps apply.",150));}for(int i=0;i<HavenMarks.Names.Length;i++){int index=i;list.Add(new HavenShopEntry(HavenMarks.Names[i],0,()=>HavenMarks.CreateReward(index),HavenMarks.Descriptions[i],HavenMarks.Prices[i]));}
