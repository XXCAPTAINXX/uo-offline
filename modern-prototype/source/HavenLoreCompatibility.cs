...
namespace Server.HavenPrototype {
 internal static class HavenLoreCompatibility {
  ...
  public static string RarityDescription(int tier){switch(tier){case 1:return "+10% starting stats/HP; +5 wild combat skills; tamed Tactics/Wrestling/Anatomy 60-69.9, other skills 0-5; caps 100 unless raised by scrolls; strengthened species signature, no training cost";case 2:return "+20% starting stats/HP; +10 wild combat skills; tamed Tactics/Wrestling/Anatomy 60-69.9, other skills 0-5; caps 100 unless raised by scrolls; stronger signature and species-specific secondary effects, no training cost";case 3:return "Found with 1 follower slot; +30% starting stats/HP; +15 wild combat skills; 3-5 random skill caps at 125-150; tamed Tactics/Wrestling/Anatomy 60-69.9, other skills 0-5; other tamed caps 100 until raised by scrolls; strongest species signature, no training cost";default:return "Standard species stats and abilities";}}
  public static void Button(Gump g,int x,int y,int width,int id,string label){for(int offset=0;offset<width;offset+=19)g.AddButton(x+Math.Min(offset,width-19),y,210,210,id,GumpButtonType.Reply,0);g.AddImageTiled(x,y,width,19,5058);g.AddHtml(x,y+1,width,18,"<CENTER><BASEFONT COLOR=#FFFFFF>"+HavenMenuText.Encode(label)+"</BASEFONT></CENTER>",false,false);}
 }
}
