...
namespace Server.HavenPrototype {
 internal static class HavenLoreCompatibility {
  ...
  public static void OpenTraining(Mobile p,BaseCreature pet){if(!HavenAnimalLoreGump.CanInspect(p,pet)||!Owned(p,pet))return;if(PetTrainingHelper.Enabled&&p is PlayerMobile)BaseGump.SendGump(new NewAnimalLoreGump((PlayerMobile)p,pet));else p.SendGump(new AnimalLoreGump(pet));}
