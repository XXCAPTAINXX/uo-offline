...
namespace Server.HavenPrototype {
 public class HavenWallet:Item {
  ...
  public override void OnDoubleClick(Mobile p){if(CanUse(p)){if(!MergeLegacy())p.SendMessage("Some old wallet gold is awaiting bank space; it remains safely stored. Make bank space and reopen your wallet.");p.SendGump(new HavenWalletGump(this,p));}}
  public override void GetProperties(ObjectPropertyList list){base.GetProperties(list);list.Add("Bank gold: "+Balance.ToString("N0"));list.Add("Double-click: bank deposits, withdrawals and tithing");}
