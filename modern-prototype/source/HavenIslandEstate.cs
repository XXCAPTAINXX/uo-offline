...
namespace Server.HavenPrototype {
 public class HavenIslandEstate:Castle {
  ...
  public HavenIslandEstate(Mobile owner):base(owner){Public=false;_ownerAccount=(owner.Account as Account)?.Username;Vault=new HavenHomeChest{Name="Island private stores",MaxItems=3000};Vault.Internalize();}
  public HavenIslandEstate(Serial serial):base(serial){}
  public static HavenIslandEstate BuildTest(Mobile owner){if(!File.Exists("ISLAND-TEST-ONLY")||owner==null||!(owner.Account is Account))throw new InvalidOperationException("Estate requires isolated test and an owner account.");ArrayList moved;var spot=new Point3D(4196,2868,0);if(HousePlacement.Check(owner,0x7E,spot,out moved)!=HousePlacementResult.Valid||moved.Count!=0)throw new InvalidOperationException("Estate plot obstructed");var estate=new HavenIslandEstate(owner);estate.MoveToWorld(spot,Map.Trammel);try{estate.InstallStores();return estate;}catch{estate.Delete();throw;}}
 ...
 public class HavenEstateStorage:Item {
  public HavenIslandEstate Estate;
  public HavenEstateStorage(HavenIslandEstate estate):base(0xE43){Estate=estate;Movable=false;Hue=0x972;Name="Connected estate stores";}
  public HavenEstateStorage(Serial serial):base(serial){}
  public override void OnDoubleClick(Mobile p){if(Estate!=null&&Estate.CanAccessStores(p,this))p.SendGump(new HavenEstateStorageGump(this,0));else p.SendMessage("These stores belong to the estate owner's account. Stand beside a storage chest to use them.");}
  public override bool OnDragDrop(Mobile p,Item item){return Estate!=null&&Estate.Deposit(p,this,item);}
  public void TargetDeposit(Mobile p){if(Estate==null||!Estate.CanAccessStores(p,this))return;p.SendMessage("Target an item in your backpack for the connected estate stores.");p.Target=new StoreTarget(this);}
  class StoreTarget:Target {readonly HavenEstateStorage _station;public StoreTarget(HavenEstateStorage s):base(3,false,TargetFlags.None){_station=s;}protected override void OnTarget(Mobile p,object target){p.SendMessage(_station.Estate!=null&&_station.Estate.Deposit(p,_station,target as Item)?"Stored. Available from every estate storage chest.":"Cannot store that item: check ownership, distance and capacity.");_station.OnDoubleClick(p);}}
  public override void Serialize(GenericWriter w){base.Serialize(w);w.Write(0);w.Write(Estate);}
  public override void Deserialize(GenericReader r){base.Deserialize(r);r.ReadInt();Estate=r.ReadItem() as HavenIslandEstate;}
 }
 public class HavenEstateStorageGump:HavenMenuGump {
  ...
  public HavenEstateStorageGump(HavenEstateStorage station,int page):base(60,60){_station=station;var all=station.Estate.Vault.Items.Where(i=>!i.Deleted).OrderBy(i=>i.Name??i.GetType().Name).ThenBy(i=>i.Serial.Value).ToArray();int pages=Math.Max(1,(all.Length+7)/8);_page=Math.Max(0,Math.Min(page,pages-1));_items=all.Skip(_page*8).Take(8).ToArray();AddBackground(0,0,610,465,3000);AddLabel(24,22,0,"CONNECTED ESTATE STORES | "+all.Length+" entries");AddLabel(24,55,0,"Private to your account. All estate chests share these items.");FlatButton(24,90,220,3,"Deposit from backpack");for(int i=0;i<_items.Length;i++){var item=_items[i];string name=item.Name??item.GetType().Name;if(name.Length>42)name=name.Substring(0,39)+"...";AddLabel(24,135+i*31,0,name+" x"+item.Amount);FlatButton(450,135+i*31,130,100+i,"Withdraw");}if(_page>0)FlatButton(24,408,120,1,"Previous");AddLabel(174,408,0,"Page "+(_page+1)+" / "+pages);if(_page+1<pages)FlatButton(316,408,120,2,"Next");FlatButton(450,408,130,0,"Close");}
  public override void OnResponse(NetState state,RelayInfo info){var p=state.Mobile;if(info.ButtonID==0||_station.Estate==null||!_station.Estate.CanAccessStores(p,_station))return;int id=info.ButtonID;if(id==3){_station.TargetDeposit(p);return;}if(id>=100&&id<100+_items.Length)p.SendMessage(_station.Estate.Withdraw(p,_station,_items[id-100])?"Item withdrawn.":"Item unavailable, or your backpack is full.");p.SendGump(new HavenEstateStorageGump(_station,_page+(id==1?-1:id==2?1:0)));}
 }
}
