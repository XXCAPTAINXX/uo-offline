...
public partial class HavenSignatureField : BaseAddon
{
    ...
    [Constructable] public HavenSignatureField():this(null,null,false,0) { }
    ...
    internal void Tick()
    {
        ...
        if(_grove)
        {
            var patients=new List<Mobile>();
            if(_owner.InRange(this,3)) { patients.Add(_owner); }
            foreach(var pet in HavenPetSignatures.NearbyCreatures(Map,Location,3))
        ...
        {
            var enemies=new List<BaseCreature>();
            foreach(var enemy in HavenPetSignatures.NearbyCreatures(Map,Location,_tier>=2 ? 2 : 1)) { enemies.Add(enemy); }
    ...
    public HavenSignatureField(Serial serial):base(serial){}
...
public partial class HavenPetHex : Item
{
    ...
    [Constructable] public HavenPetHex():base(1) { Name="temporary pet effect";Visible=false;Movable=false;Weight=0; }
    ...
    internal static bool Apply(BaseCreature pet,BaseCreature target,int kind,int power,int seconds)
    {
        ...
        foreach(HavenPetHex existing in target.Backpack.FindItemsByType(typeof(HavenPetHex),true))
        { if(existing._kind==kind) { previous=existing;break; } }
        if(previous!=null)
        { if(DateTime.UtcNow<previous._expires) { return false; } previous.Delete(); }
        var record=new HavenPetHex { _pet=pet,_target=target,_owner=pet.ControlMaster,_kind=kind,_expires=DateTime.UtcNow+TimeSpan.FromSeconds(seconds) };
        target.Backpack.DropItem(record);
        if(kind==0)
        { record._resistance=new ResistanceMod(ResistanceType.Physical,-power);target.AddResistanceMod(record._resistance); }
        else
    ...
    internal void Tick()
    {
        if(DateTime.UtcNow>=_expires || !HavenPetSignatures.Active(_pet) || _pet.ControlMaster!=_owner || _target?.Deleted!=false ||
            !_target.Alive || _target.Controlled || _target.Map!=_pet.Map || !_target.InRange(_pet,18)) { Delete(); }
    }
    public HavenPetHex(Serial serial):base(serial){}
    public override void Serialize(GenericWriter w){base.Serialize(w);w.Write(0);}
    public override void Deserialize(GenericReader r){base.Deserialize(r);r.ReadInt();Timer.DelayCall(TimeSpan.Zero,Delete);}
