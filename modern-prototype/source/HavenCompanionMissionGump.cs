...
namespace Server.HavenPrototype
{
    public class CompanionActivityGump:HavenMenuGump
    {
        ...
        {
            _companion=companion;_tab=Math.Max(0,Math.Min(2,tab));_minutes=minutes==15||minutes==30?minutes:5;
            ...
            AddBackground(0,0,720,540,0xA28);
            ...
            if(_tab!=2){Text(24,130,108,25,"Duration");Button(138,130,20,_minutes==5?"5 min [x]":"5 min",100);Button(303,130,21,_minutes==15?"15 min [x]":"15 min",110);Button(478,130,22,_minutes==30?"30 min [x]":"30 min",110);}
        ...
        public override void OnResponse(NetState state,RelayInfo info)
        {
            ...
            if(id==1){bool ok=_tab==2?_companion.SetRole(p,(CompanionRole)_selection):_companion.StartMission(p,_minutes,(CompanionMission)(_tab==1?_selection+6:_selection));if(ok&&_tab!=2){_companion.Show(p);return;}if(!ok&&_tab==2)p.SendMessage("Move near your companion and finish combat before changing roles.");}
            ...
            int minutes=id==20?5:id==21?15:id==22?30:_minutes;
