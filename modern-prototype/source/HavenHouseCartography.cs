...
namespace Server.HavenPrototype
{
    ...
    public class HavenMapStorageGump:HavenMenuGump{
        readonly HavenMapStorageChest _chest;readonly Item[] _items;readonly string _search;readonly int _page;readonly int _sort;readonly bool _descending;
        public static Item[] SortedItems(IEnumerable<Item> items,string search,int sort,bool descending)
        {
            var matches=items.Where(x=>!x.Deleted&&Description(x).IndexOf(search??"",StringComparison.OrdinalIgnoreCase)>=0);
            IOrderedEnumerable<Item> ordered;
            if(sort==1) ordered=descending?matches.OrderByDescending(x=>x is TreasureMap?((TreasureMap)x).Level:-1):matches.OrderBy(x=>x is TreasureMap?((TreasureMap)x).Level:-1);
            else if(sort==2) ordered=descending?matches.OrderByDescending(FacetName,StringComparer.OrdinalIgnoreCase):matches.OrderBy(FacetName,StringComparer.OrdinalIgnoreCase);
            else if(sort==3) ordered=descending?matches.OrderByDescending(Kind):matches.OrderBy(Kind);
            else ordered=descending?matches.OrderByDescending(Description,StringComparer.OrdinalIgnoreCase):matches.OrderBy(Description,StringComparer.OrdinalIgnoreCase);
            return ordered.ThenBy(Description,StringComparer.OrdinalIgnoreCase).ThenBy(x=>x.Serial.Value).ToArray();
        }
        static int Kind(Item item){return item is TreasureMap?0:item is SOS?1:2;}
        static string FacetName(Item item){var map=item as TreasureMap;if(map!=null)return map.Facet==null?"":map.Facet.Name;var sos=item as SOS;return sos==null||sos.TargetMap==null?"":sos.TargetMap.Name;}
        ...
        public HavenMapStorageGump(HavenMapStorageChest chest,string search="",int page=0,int sort=0,bool descending=false):base(50,50){_chest=chest;_sort=Math.Max(0,Math.Min(3,sort));_descending=descending;_search=(search??"").Substring(0,Math.Min(60,(search??"").Length));_items=SortedItems(chest.Items,_search,_sort,_descending);_page=Math.Max(0,Math.Min(page,Math.Max(0,(_items.Length-1)/10)));AddBackground(0,0,670,585,3000);AddLabel(24,20,0,"Cartographer's treasure-map chest");AddLabel(24,51,0,"Search by facet, level, coordinates, SOS or bottle. Select to withdraw.");AddBackground(24,80,405,30,0xBB8);AddTextEntry(32,85,388,22,0,0,_search);FlatButton(450,85,194,1,"Search");FlatButton(24,120,145,6,(_sort==1?"[Level]":"Level"));FlatButton(184,120,145,7,(_sort==2?"[Facet]":"Facet"));FlatButton(344,120,145,8,(_sort==3?"[Type]":"Type"));FlatButton(504,120,140,9,_descending?"Descending":"Ascending");for(int row=0;row<10;row++){int index=_page*10+row;if(index>=_items.Length)break;FlatButton(24,170+row*29,620,100+index,Description(_items[index]));AddItemProperty(_items[index].Serial);}FlatButton(24,476,190,2,"Collect from my pack");FlatButton(230,476,194,3,"Target item / bag...");FlatButton(440,476,204,4,"Previous");FlatButton(24,534,190,5,"Next");AddLabel(246,534,0,"Page "+(_page+1)+" / "+Math.Max(1,(_items.Length+9)/10));FlatButton(544,534,100,0,"Close");}
        public override void OnResponse(NetState state,RelayInfo info){var p=state.Mobile;if(info.ButtonID==0||!_chest.CanUse(p))return;string search=info.GetTextEntry(0)?.Text??_search;int page=_page;int sort=_sort;bool descending=_descending;if(info.ButtonID>=6&&info.ButtonID<=9){if(info.ButtonID==9)descending=!descending;else sort=info.ButtonID-5;page=0;}else if(info.ButtonID==1)page=0;else if(info.ButtonID==2)p.SendMessage("Stored "+_chest.Collect(p,p.Backpack)+" maps/messages.");else if(info.ButtonID==3){p.Target=new CollectTarget(_chest,search,_sort,_descending);return;}else if(info.ButtonID==4)page--;else if(info.ButtonID==5)page++;else if(info.ButtonID>=100&&info.ButtonID-100<_items.Length&&!_chest.Withdraw(p,_items[info.ButtonID-100]))p.SendMessage("Unable to withdraw; check backpack space.");p.SendGump(new HavenMapStorageGump(_chest,search,page,sort,descending));}
        sealed class CollectTarget:Target{readonly HavenMapStorageChest _chest;readonly string _search;readonly int _sort;readonly bool _descending;public CollectTarget(HavenMapStorageChest chest,string search,int sort,bool descending):base(-1,false,TargetFlags.None){_chest=chest;_search=search;_sort=sort;_descending=descending;}protected override void OnTarget(Mobile p,object target){p.SendMessage("Stored "+_chest.Collect(p,target as Item)+" maps/messages.");if(_chest.CanUse(p))p.SendGump(new HavenMapStorageGump(_chest,_search,0,_sort,_descending));}}
    }
}
