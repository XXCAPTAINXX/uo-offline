...
namespace Server.HavenPrototype {
 public class HavenChampionCodex:Bag {
  ...
  public override bool CheckHold(Mobile p,Item item,bool message,bool checkItems,int plusItems,int plusWeight){return CanUse(p)&&Accepts(item);}
  ...
  public void Show(Mobile p,int page=0){if(CanUse(p)){p.CloseGump(typeof(HavenCodexGump));p.CloseGump(typeof(HavenCodexBrowserGump));p.CloseGump(typeof(HavenCodexCellGump));p.SendGump(new HavenCodexGump(p,this,page));}}
  ...
  public override void GetProperties(ObjectPropertyList list){base.GetProperties(list);list.Add("Archives scrolls, champion skulls, mastery primers and binders");list.Add("Unlimited storage; contents add no weight or backpack slots");list.Add("Collect nested bags; withdraw, combine or split scrolls");}
  public override void Serialize(GenericWriter w){base.Serialize(w);w.Write(0);}
  public override void Deserialize(GenericReader r){base.Deserialize(r);r.ReadInt();MaxItems=0;}
 }
}
