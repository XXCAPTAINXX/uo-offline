...
namespace Server.HavenPrototype
{
    ...
    public class HavenMiniPrize : Container
    {
        ...
        public HavenMiniPrize(Mobile owner,int theme,bool islandRewards=false):base(0xE76){_owner=owner;Movable=false;Visible=false;var whirlwind=WhirlwindReward(Utility.RandomDouble(),Utility.Random(3));if(whirlwind!=null)DropItem(whirlwind);DropItem(new BankCheck(10000));if(islandRewards){DropItem(new MaritimeCargo(Utility.RandomDouble()<0.20?CargoQuality.Legendary:CargoQuality.Exalted));}HavenMiniResources.Completion(this);for(int i=0;i<5;i++)DropItem(PowerScroll.CreateRandomNoCraft(5,10));DropItem(new ScrollOfAlacrity(theme==0?SkillName.Lumberjacking:theme==1?SkillName.Tactics:SkillName.MagicResist));DropItem(new ScrollOfTranscendence(theme==0?SkillName.Lumberjacking:theme==1?SkillName.Tactics:SkillName.MagicResist,Utility.RandomMinMax(5,20)/10.0));}
