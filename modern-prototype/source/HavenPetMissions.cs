...
namespace Server.HavenPrototype
{
    public static class HavenPetMissions
    {
        ...
        public static void ApplyRarity(BaseCreature pet,int tier)
        {
            ...
            foreach(var skill in new[]{SkillName.Wrestling,SkillName.Tactics,SkillName.MagicResist,SkillName.Magery,SkillName.EvalInt})if(pet.Skills[skill].Base>0){pet.Skills[skill].Cap=Math.Max(pet.Skills[skill].Cap,100);pet.Skills[skill].Base=Math.Min(pet.Skills[skill].Cap,pet.Skills[skill].Base+tier*5);}
    ...
    public partial class HavenCompanion
    {
        ...
        void CompletePetMission(){if(_scheduledPet==null||_scheduledPet.Deleted)return;var supplies=_scheduledPet.TakeSupplies();if(supplies!=null){if(supplies.Items.Count>0){Backpack.DropItem(supplies);UnpackMissionSupplies();}else supplies.Delete();}_pendingPets.Add(_scheduledPet);_scheduledPet=null;HavenCompanionProgression.TamingBardPractice(this,_missionMinutes);foreach(var skill in new[]{Skills.AnimalTaming,Skills.AnimalLore})if(skill.Lock==SkillLock.Up)skill.BaseFixedPoint=Math.Min(skill.CapFixedPoint,skill.BaseFixedPoint+HavenCompanionProgression.TamingTraining(skill.BaseFixedPoint,_missionMinutes,Utility.RandomDouble()));}
    ...
    public class HavenPetMissionGump:HavenPetMenuGump
    {
        ...
        public HavenPetMissionGump(HavenCompanion c,int minutes=5):base(55,55){_companion=c;_minutes=minutes;AddBackground(0,0,610,570,0xA28);AddLabel(24,20,0,"Companion taming missions");AddLabel(24,52,0,"Taming "+c.Skills.AnimalTaming.Base.ToString("0.0")+" / Lore "+c.Skills.AnimalLore.Base.ToString("0.0")+"; both must meet the requirement.");Button(24,89,1,"5 min");Button(160,89,2,"15 min");Button(300,89,3,"30 min");AddLabel(436,89,0,"Selected: "+minutes+" min");for(int i=0;i<12;i++){int y=130+i*29;Button(24,y,100+i,HavenPetMissions.Names[i]);AddLabel(370,y,0,"Needs "+HavenPetMissions.Requirements[i].ToString("0.0")+" both");}AddLabel(24,489,0,"Completed trips train Taming, Lore, Peacemaking and Musicianship.");Button(24,530,4,"Collect pending ("+c.PendingPetTickets+")");Button(436,530,0,"Back");}
