...
namespace Server.HavenPrototype
{
    ...
    public partial class HavenCompanion
    {
        ...
        void CompletePetMission(){if(_scheduledPet==null||_scheduledPet.Deleted)return;var supplies=_scheduledPet.TakeSupplies();if(supplies!=null){if(supplies.Items.Count>0){Backpack.DropItem(supplies);UnpackMissionSupplies();}else supplies.Delete();}_pendingPets.Add(_scheduledPet);_scheduledPet=null;foreach(var skill in new[]{Skills.AnimalTaming,Skills.AnimalLore})skill.BaseFixedPoint=Math.Min(skill.CapFixedPoint,skill.BaseFixedPoint+HavenCompanionProgression.TamingTraining(skill.BaseFixedPoint,_missionMinutes,Utility.RandomDouble()));}
