...
public static class StoryVoiceSmoke
{
 ...
 public static void Run(Action<string> log,Action done)
 {
  ...
  try{HavenBeaconQuest.Speak(p,0);HavenBeaconQuest.Speak(p,5);HavenBeaconQuest.Speak(p,6);HavenBeaconQuest.Speak(p,12);HavenBeaconQuest.Speak(p,5);}
  catch(Exception e){log("FAIL "+e);finish();return;}
  Timer.DelayCall(TimeSpan.FromSeconds(2.1),()=>{try{
   if(!net.Sounds.SequenceEqual(new[]{32750,32755,32756,32762}))throw new Exception("Voice sequence: "+String.Join(",",net.Sounds));
   ...
   Timer.DelayCall(TimeSpan.FromSeconds(0.8),()=>{try{if(net.Sounds.Count!=atMute)throw new Exception("Muted queue packets: "+String.Join(",",net.Sounds));log("PASS captured actual voice packets: greeting, cape, horse and chapter completion in order; duplicate queued line suppressed; mute clears later pending audio");}catch(Exception e){log("FAIL "+e);}finally{finish();}});
