...
public static class HouseStorageBrowserSmoke
{
 public static void Run(PlayerMobile owner,Action<string> log)
 {
  ...
  try {
   ...
   foreach(int sort in new[]{0,1,2}){var g=new HavenEstateStorageGump(station,999,null,"",0,sort);if(g.Entries.Count==0)throw new Exception("Empty gump");}
   if(new HavenEstateStorageGump(station,0,null,"no-such-search",2,0).Entries.Count==0)throw new Exception("Empty search gump");
   log("PASS protected nested loot deposit, foreign access rejection, full-storage retention, nested withdrawal, stale replay, browser sort/empty/page construction");
