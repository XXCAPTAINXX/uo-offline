...
var bag=new Bag();var gold=new Gold(200);bag.DropItem(gold);c.Backpack.DropItem(bag);
 ...
 Check(!camp.Active,"challenge completes only after all bosses");Check(HavenMarks.Balance(owner)==marksBeforeChallenge+80,"challenge awards eighty Marks");Check(owner.Backpack.FindItemsByType(typeof(BankCheck),true).Cast<BankCheck>().Sum(x=>x.Worth)==40000,"challenge awards four gold reward bundles");var pirate=new HavenMiniPrize(owner,1);Check(pirate.FindItemByType(typeof(Cannonball))!=null&&pirate.FindItemByType(typeof(PowderCharge))!=null&&pirate.FindItemByType(typeof(FuseCord))!=null,"native ship ammunition in corsair prize");pirate.Delete();camp.Delete();
