...
public static class CompanionInventorySmoke {
 ...
 owner.Criminal=true;Check(Server.Spells.SpellHelper.CheckCanTravel(owner),"native travel helper permits criminal-only travel");var gate=new PublicMoongate();Check(gate.CanUseGate(owner,false),"public moongate permits criminal-only travel");gate.Delete();owner.Criminal=false;
 var trash=new HavenTrashBag();owner.Backpack.DropItem(trash);var scrap=new Katana();scrap.Attributes.WeaponDamage=20;scrap.WeaponAttributes.HitLeechHits=50;scrap.WeaponAttributes.HitLeechMana=50;double value=Server.Engines.Points.CleanUpBritanniaData.GetPoints(scrap);Check(value>0,"scrap has native CUB value");double balance=Server.Engines.Points.PointsSystem.CleanUpBritannia.GetPoints(owner);
 Check(trash.OnDragDrop(owner,scrap),"trash accepts eligible scrap");Check(Server.Engines.Points.PointsSystem.CleanUpBritannia.GetPoints(owner)==balance,"no CUB credit before disposal");owner.Backpack.DropItem(scrap);trash.Empty(501479);Check(Server.Engines.Points.PointsSystem.CleanUpBritannia.GetPoints(owner)==balance,"retrieved trash earns no points");
 Check(trash.OnDragDrop(owner,scrap),"retrieved item can be discarded again");using(var stream=new MemoryStream()){var writer=new BinaryFileWriter(stream,true);trash.WritePendingCleanup(writer);writer.Flush();trash.m_Cleanup.Clear();stream.Position=0;trash.ReadPendingCleanup(new BinaryFileReader(new BinaryReader(stream)));}Check(trash.m_Cleanup.Count==1,"pending CUB ownership and value survive serialization");trash.Empty(501479);Check(Server.Engines.Points.PointsSystem.CleanUpBritannia.GetPoints(owner)==balance+value,"disposed trash earns native CUB points once");trash.Empty(501479);Check(Server.Engines.Points.PointsSystem.CleanUpBritannia.GetPoints(owner)==balance+value,"empty trash cannot award twice");trash.Delete();var publicTrash=new HavenPublicTrashChest();publicTrash.MoveToWorld(owner.Location,owner.Map);var publicScrap=new Katana();publicScrap.WeaponAttributes.HitLeechMana=50;publicScrap.WeaponAttributes.HitLeechHits=50;var nestedTrash=new Bag();nestedTrash.DropItem(publicScrap);double publicValue=Server.Engines.Points.CleanUpBritanniaData.GetPoints(publicScrap);double publicBefore=Server.Engines.Points.PointsSystem.CleanUpBritannia.GetPoints(owner);Check(publicTrash.OnDragDropInto(owner,nestedTrash,new Point3D(40,40,0)),"public trash accepts nested scrap via open container");owner.Backpack.DropItem(publicScrap);Check(publicTrash.m_Cleanup.Count==0,"retrieving nested scrap cancels pending credit");publicTrash.Empty(501479);Check(publicTrash.OnDragDrop(owner,publicScrap),"public trash accepts direct drop");publicTrash.Empty(501479);Check(Server.Engines.Points.PointsSystem.CleanUpBritannia.GetPoints(owner)==publicBefore+publicValue,"public trash credits depositor once");publicTrash.Delete();
 var c=HavenCompanion.Claim(owner);
 Check(HavenCompanionStatsGump.Rows(c,2,0,new double[c.Skills.Length]).Length==c.Skills.Length,"all-skills view includes every skill across pages");var skillScreen=new HavenCompanionStatsGump(c);Check((int)typeof(HavenCompanionStatsGump).GetField("_filter",System.Reflection.BindingFlags.Instance|System.Reflection.BindingFlags.NonPublic).GetValue(skillScreen)==2,"skill screen defaults to all skills");
...
var bag=new Bag();var gold=new Gold(200);bag.DropItem(gold);c.Backpack.DropItem(bag);
 ...
 Check(!camp.Active,"challenge completes only after all bosses");Check(HavenMarks.Balance(owner)==marksBeforeChallenge+80,"challenge awards eighty Marks");Check(owner.Backpack.FindItemsByType(typeof(AstralShard),true).Sum(x=>x.Amount)==1,"challenge awards one guaranteed bonus Astral Shard");Check(owner.Backpack.FindItemsByType(typeof(BankCheck),true).Cast<BankCheck>().Sum(x=>x.Worth)==40000,"challenge awards four gold reward bundles");var pirate=new HavenMiniPrize(owner,1);Check(pirate.FindItemByType(typeof(Cannonball))!=null&&pirate.FindItemByType(typeof(PowderCharge))!=null&&pirate.FindItemByType(typeof(FuseCord))!=null,"native ship ammunition in corsair prize");pirate.Delete();camp.Delete();
