...
{
 static void StartingSkills(BaseCreature pet)
 {
  for(int i=0;i<pet.Skills.Length;i++){var name=(SkillName)i;var n=pet.Skills[i].BaseFixedPoint;bool combat=name==SkillName.Tactics||name==SkillName.Wrestling||name==SkillName.Anatomy;if(combat?(n<600||n>699):(n<0||n>50))throw new Exception("Wrong starting skill "+name+"="+n);}
 }
 public static void Run(Action<string> log)
 {
  for(int tier=0;tier<=3;tier++)
  {
   ...
   try
   {
    ...
    AnimalTaming.ScaleSkills(pet,.9,true);StartingSkills(pet);
    ...
    if(record!=null){HavenLegendaryPetSkills.Roll(pet);StartingSkills(pet);if(rolls.Any(n=>pet.Skills[n].Base>90.001))throw new Exception("Repair restored trained levels");
     ...
     using(var stream=new System.IO.MemoryStream()){var w=new BinaryFileWriter(stream,true);record.Serialize(w);w.Flush();stream.Position=0;record.Deserialize(new BinaryFileReader(new System.IO.BinaryReader(stream)));}HavenLegendaryPetSkills.Roll(pet);StartingSkills(pet);if(rolls.Any(n=>pet.Skills[n].Base>90.001))throw new Exception("Reload repair erased training");
    }
  ...
  log("PASS all rarities start with Tactics/Wrestling/Anatomy60-69.9 and other skills0-5, ordinary caps100, random caps retained, support skills reduced, repair/reload preserves training, retame preserves purchased caps, mission tickets follow same rules");
 }
}
