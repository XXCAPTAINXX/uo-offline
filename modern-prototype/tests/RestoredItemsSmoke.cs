...
public static class RestoredItemsSmoke {
 public static void Run(Mobile p,HavenCompanion c,Action<bool,string> check){
  ...
  check(!HavenPetTrainingMenu.Peaceful(p,c),"recent companion combat blocks training");typeof(AggressorInfo).GetField("m_LastCombatTime",System.Reflection.BindingFlags.Instance|System.Reflection.BindingFlags.NonPublic).SetValue(combatRecord,DateTime.UtcNow.AddSeconds(-16));check(HavenPetTrainingMenu.Peaceful(p,c)&&!combatRecord.Expired,"training clears after fifteen seconds despite lingering target and native aggression");combatRecord.Refresh();check(!HavenPetTrainingMenu.Peaceful(p,c),"new pet attack resets short cooldown");c.Aggressed.Remove(combatRecord);c.Combatant=null;combatFoe.Delete();
  ...
  var ordinary=new Horse();ordinary.SetControlMaster(p);ordinary.MoveToWorld(p.Location,p.Map);var stored=HavenPetTicket.Store(ordinary,p,p.Backpack);check(stored!=null&&HavenPetExchange.Eligible(p,stored)&&HavenPetExchange.Exchange(p,stored),"stored ordinary pet exchanges once");stored.Delete();var legendary=new HavenPetTicket(p,6,5,3);p.Backpack.DropItem(legendary);check(!HavenPetExchange.Eligible(p,legendary)&&!HavenPetExchange.Exchange(p,legendary)&&!legendary.Deleted,"legendary tickets protected from turn-in");var roll=legendary.Pet.Backpack.FindItemByType(typeof(HavenLegendaryPetSkills),true) as HavenLegendaryPetSkills;check(roll!=null&&roll.Boosted.Count()>=3&&roll.Boosted.Count()<=5,"legendary has three to five skills");legendary.Delete();
