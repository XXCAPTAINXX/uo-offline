...
public static class AfkMissionSmoke {
 public static void Run(Action<string> log){
  var owner=new PlayerMobile{Player=true,Body=0x190,RawStr=250};owner.AddItem(new Backpack());new Account("afk-"+Guid.NewGuid().ToString("N"),Guid.NewGuid().ToString("N"))[0]=owner;owner.MoveToWorld(new Point3D(1015,527,-65),Map.Malas);
  ...
  try{
   ...
   plan.AutoAfk=true;plan.Tick();if(!c.OnMission)throw new Exception("Automatic AFK did not dispatch");
   ...
   log("PASS connected manual and automatic AFK dispatch; no duplicate trip; finish/recall on return; activity reset; disabled plan and auto switch; five-minute threshold; settings construction");
