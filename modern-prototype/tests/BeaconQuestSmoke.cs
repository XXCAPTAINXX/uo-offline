...
public static class BeaconQuestSmoke
{
 ...
 public static void Run(Action<string> log)
 {
  ...
  try
  {
   ...
   c.Skills.Peacemaking.SetLockNoRelay(SkillLock.Up);c.Skills.Musicianship.SetLockNoRelay(SkillLock.Up);c.Skills.Peacemaking.Cap=120;c.Skills.Musicianship.Cap=120;c.Skills.Peacemaking.Base=80;c.Skills.Musicianship.Base=80;
   HavenCompanionProgression.TamingBardPractice(c,5);Check(c.Skills.Peacemaking.BaseFixedPoint==803&&c.Skills.Musicianship.BaseFixedPoint==801,"Taming bard practice rate");
   c.Skills.Peacemaking.SetLockNoRelay(SkillLock.Locked);c.Skills.Musicianship.Base=119.9;HavenCompanionProgression.TamingBardPractice(c,30);Check(c.Skills.Peacemaking.BaseFixedPoint==803&&c.Skills.Musicianship.BaseFixedPoint==1200,"Practice lock/cap");
   HavenCompanionProgression.TamingBardPractice(c,0);Check(c.Skills.Peacemaking.BaseFixedPoint==803&&c.Skills.Musicianship.BaseFixedPoint==1200,"Zero duration practice");
   horse.IsBonded=false;horse.MoveToWorld(p.Location,p.Map);var protectedBook=HavenPetBook.Ensure(p);var protectedTicket=HavenPetTicket.Store(horse,p,protectedBook);Check(protectedTicket!=null,"Starter horse storage");protectedTicket.Favorite=false;int protectedBalance=HavenPetExchange.Balance(p);
   Check(!HavenPetExchange.Eligible(p,protectedTicket,protectedBook)&&!HavenPetBulkExchangeConfirm.Eligible(p,protectedTicket,protectedBook)&&!HavenPetExchange.Exchange(p,protectedTicket,protectedBook)&&!horse.Deleted&&HavenPetExchange.Balance(p)==protectedBalance,"Starter horse exchange protection");Check(protectedTicket.Claim(p),"Protected horse still claimable");
   log("PASS taming-mission bard practice rate/locks/caps and unbonded, unfavorited starter horse protected from single/bulk exchange while still claimable");
   TamingMissionPracticeSmoke.Run(log);
