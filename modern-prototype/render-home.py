...
for mode in ('exterior','ground','upper'):
    im=Image.new('RGBA',(1450,1250),'#344b39')
    selected=[]
    for t in tiles:
    ...
    for t in sorted(selected,key=lambda t:(t['x']+t['y'],t['x'],t['z'])):
        sprite=art(t['id']);x=725+(t['x']-t['y'])*22-sprite.width//2;y=530+(t['x']+t['y'])*22-t['z']*4-sprite.height+44
        im.alpha_composite(sprite,(x,y))
    d=ImageDraw.Draw(im);f=ImageFont.truetype('C:/Windows/Fonts/segoeui.ttf',24)
    d.text((25,22),'Pirate lodge - '+mode+' | actual modern tile art; '+('full exterior' if mode=='exterior' else 'front walls / upper roof hidden'),font=f,fill='white')
