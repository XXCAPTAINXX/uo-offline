...
Player screenshot follow-up: client renders greater-than entity literally and bold HTML as blotchy text. Replaced entity with Selected: and removed bold tags from mission menu. Staged for next restart; native build passed. Pack weight verified: backpack own weight3 plus contents27 equals displayed total30; no weight accounting change needed.

Inventory and menu release: native pet loyalty/release excluded for persistent companions; DropBackpack defensive override. Runtime checks passed all five roles, retained nested item identities through dispatch/early recall, 1000-item unlimited-weight pack, and unchanged normal pet release. Deployed with parchment3000 theme and direct Tame/Cancel combat action. Backup E:/Backups/Haven/Prototypes/servuo-before-inventory-fix-20260911-184508 verified by save hashes; live build and login passed. Earlier unexpected process stop recovered; cause not established. Native-art approximate menu mock reviewed at7.5/10, not client-certified.
Level tooltip follow-up STAGED ONLY: shared Equipment level X/20 | XP X/cap line for starter, jewelry, companion evolution and advanced gear through scoped native equipment property hooks patch0015. Max-level marker added; paid robe tiers remain tiers. No progression math changed. Build passed; not deployed.

Button style follow-up staged: replace metallic boxed arrows in shared parchment menus with native blue/gold diamond art2151/2152. Inspected decoded native art. Shared AddButton mapping preserves IDs and layout; ItemArrow retains property hover with new art. Build passed; next restart pending.

Offline route picker deployed: Offline setup now selects all23 routes via paginated picker and5/15/30/60 durations directly, then saves using existing ownership/proximity Configure checks. Opening without carried selection loads saved focus/duration. Cycling preserves candidate and duration. Build clean, live save backed up/hash-verified at E:/Backups/Haven/Prototypes/servuo-before-offline-picker-20260911-185422. Full rebuild/start/login probe passed. Also deployed queued gem-button theme and patch0015 leveling tooltip hooks. Actual player UI confirmation pending.

User reference combat bar: staged compact350x112 translucent dark surface, flat rectangular76x19 controls with centered text and gold active-order label. Each rectangle backed by four adjoining native19px hit areas; native tile drawn over them instead of icon beside text. No client asset replacement. Build passed; actual client hit testing/rendering still required before release.

Remote Recall verified in isolated runtime: distant same-facet and cross-facet recalls both return exact companion and retained inventory to owner; no range/LOS/same-map restriction exists. Added specific Recall failure reasons for owner/companion combat, death, stabling and destination state. Existing early mission recall also passes. Diagnostics staged, unlimited-range behavior already live.

Care/training release deployed: patch0016 protects owner care of own criminal companion; companion care of own/owner-controlled pets cannot renew shared flag. Actual companion crimes and unrelated criminal aid retained. Runtime tests passed; offence tests isolated after other checks because native crime has side effects. Faster early taming:5.0 per5min below100, baseline above100, twentyfold slowdown above120. Taming/Lore decreases blocked via OnSkillChange and logged with stack; exact prior reset trigger not established. Full live build/start/login passed, backup E:/Backups/Haven/Prototypes/servuo-before-care-training-20260911-191224.
Staged next: mission commands reopen a single timer through ShowAwayTimer, including combat bar, expanded menu, pets, pack, ledger and taming; successful recall closes timer. Reported Taming53 minimum repair helper requires opt-in marker and exactly named companion/owner; not activated or claimed restored. Away timer build clean.

Staged challenge/roles/status: mini-champ theme3 combines all variants,15 enemies each of3 waves and3 bosses; completion gives three existing reward sets plus60 Marks once. Corsair prizes include native25 cannonballs/25 powder charges/25 fuse cords; applies to existing camp and reusable for island placement (finished island not implemented). Runtime test passed all stage populations/variant names/completion and native loot types. Roles now has dedicated main-menu entry and screen, with role tab removed from Missions. Stats filter uses actual equipped weapon.GetUsedSkill, includes Bard and Taming skills, and displays current weapon skill. Bard currently unequips warrior sword, so Wrestling in player screenshot is expected; Warrior equips Swords weapon. Native build passed; these changes not deployed.

Travel stone cleanup (staged, not deployed): name becomes Travel stone; double-click opens destinations directly; removes test preparation and companion actions from travel menu, removes test-training link from travel service group, and updates login/help wording. Stale preparation button replies are rejected. Existing destinations retained. Isolated Release build passed with zero warnings/errors. Bundle with next deployment; no restart performed.

Pet ability categories (staged, not deployed): replaced mixed Abilities with native Magical abilities, Special abilities, Special moves, Area effect abilities. Eight categories fit the left panel; each retains native eligibility and its own pagination. Release build passed with zero warnings/errors; native regression reached COMPLETE including type isolation and all eight menu categories. Bundle with travel-only cleanup at next deployment.

Training hover descriptions (staged): attach native description tooltip to ability name and every hit region/overlay of the flat upgrade button. Applies to stats and caps too where native description exists. Native entries with no description ID remain without hover text. Isolated Release build passed with zero warnings/errors. Not deployed; bundle with queued categories and travel cleanup.
...
Personal stats and command guide (staged): new [mystats shows player vitals, stats, native regeneration, resists, equipment attribute totals, Luck/follower usage and all skills across pages with Base/Now/Cap plus free-skill markers and counted budget. Read-only, manual refresh. Command guide replaced with parchment, flat category tabs, grouped current commands and no test checklist entries. Includes pet exchange, pet training, Tidebound controls and personal stats. Release build zero warnings/errors; client appearance not yet verified. Not deployed.

Role skill preview (staged): companion role selection shows relevant combat/casting/support skills with trained Base, effective Now and Cap; values refresh when browsing roles. Includes shared healing/resistance and bard taming assistance. Current bonuses explicitly reflect equipped gear, not a predicted gear change. Expanded panel prevents overlap with role action. Release build zero warnings/errors. Not deployed.

2026-09-12 restart: deployed ten production files through 88bb373; see STATS-TRAINING-RESTART-20260912.md. Verified 43 save files, build clean, regression COMPLETE, login passed. Follow-up staged: Cyclone Mace/Kryss, three one-handed skill families across shop and legendary loot, plus 5% chance per mini-champ reward bundle (Challenge has four bundles). Ship supplies reserved for island rewards; Haven retains original core rewards. [mystats now sorts by trained Base descending, alphabetical ties. These follow-ups are not live.
Combat audit: HavenPreview.TravelCombatSeconds is 15 seconds. Companion SetRole, mission start and recall still check raw aggression lists/Combatant; native aggression expiry is 2 minutes. Pet training Peaceful also uses native combat and unexpired outgoing aggression plus Combatant. These inconsistent action gates are confirmed and remain to be aligned; no global crime/aggression change made.

Short companion/pet action cooldowns (staged): role changes, mission dispatch, companion recall and Haven pet training now use the shared 15-second last-combat check for both owner and creature. Lingering Combatant/native aggression records do not prolong these gates. Actual aggression/crime records preserved. Release build zero warnings/errors; regression COMPLETE including fresh attack block, aged16s unblocks, and renewed attack resets cooldown. Not deployed. User also proposed Luck shortening mission time; design pending, no Luck effect implemented.

Mission Luck (staged): 2% time reduction per100 owner Luck, capped at one-third. 60-minute tier reaches40 minutes at1667 Luck; full selected reward/skill/supply tier retained. Both manual and offline dispatch share calculation, captured as existing saved absolute mission due date; active trips unchanged. Mission menu shows proposed adjusted duration and full reward tier. Zero-warning/error build and regression COMPLETE, including cap and duration calculations. Not deployed.
