...
Roles/zero-slot checkpoint deployed 2026-09-11, backed up and login verified. Healer/Bard/caster behaviors above are live; remaining broad progression/reward parity and island work are not marked complete.
...
Island mini-champion clarification (2026-09-12): ship/pirate goods belong to the future housing-island mini champion, not Haven's mini champion. Island encounters should use pirate themes and retain valuable core rewards (gold, Marks, scrolls and gear) rather than replacing value with ship supplies. Haven prize ship supplies are now disabled by default in staged source; explicit island reward option is reserved for future island integration. Island encounter itself is not yet built.

## Low priority: Jenna voice lines (requested 2026-09-12)
- Add a female voice for Jenna's greetings, farewells, quest/mission departures and returns.
- Keep this near the bottom of the backlog, after island work and current fixes.
- Choose an original or appropriately licensed voice; the user has not requested imitation of his wife's voice.
- Keep lines occasional, with a mute option, and use the client sound support.
- Backlog only: no voice assets commissioned, generated or installed yet.
